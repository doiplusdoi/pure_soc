...
`PURESOC_PERSISTENCE_MODE=memory` is the deterministic default used by tests and local contract runs. The live Compose catalog sets the role-specific runtime values needed for the bundled stack, including `PURESOC_PERSISTENCE_MODE=prisma` and an internal `DATABASE_URL` for API, scheduler, and migrator containers. `.env.example` contains only live deployment secrets that cannot be generated by the repository.
...
Dockerfiles under `infra/docker/` run workspace entrypoint scripts. API, web, and report-renderer start implemented HTTP processes. Worker, scheduler, and connector-runner start typed job-runtime loops. The Compose migrator reuses the API image to run `pnpm prisma:migrate:deploy`, the live Compose API service defensively runs that same idempotent migration command before `pnpm start:api`, and API startup runs `npm run prisma:migrate:deploy` in Prisma mode before it binds the HTTP port.
