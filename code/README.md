...
The default Compose path gives migration ownership to the one-shot `puresoc-migrator`, which runs `pnpm prisma:migrate:deploy` before the API starts. Compose sets `PURESOC_API_SKIP_STARTUP_MIGRATIONS=true` for `puresoc-api`, pins `PORT=3001`, and starts web only after the API healthcheck passes. The API process still runs `npm run prisma:migrate:deploy` before listening in Prisma mode when it is launched outside Compose without that override. None of these paths resets the database or removes the named `puresoc-postgres-data` volume.
