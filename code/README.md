...
`compose.yml` is the local/in-a-box Docker entry point. It sets Prisma/Postgres persistence, Redis-backed queues and rate limits, bundled MinIO object storage, ClamAV upload scanning, internal report rendering, Origin/Referer enforcement, Stripe disabled, social login disabled, and Microsoft 365 connector `auto` mode. A clean checkout starts without Microsoft secrets by using the fixture-backed connector path; when real connector credentials are supplied, `auto` switches to live read-only Microsoft 365 admin-consent. The API is not published by Compose; `puresoc-web` calls `http://puresoc-api:3001` over Docker networking.

The default profile is browser-usable on `http://127.0.0.1:3000`: `PURESOC_APP_ENV=development`, `PURESOC_AUTH_COOKIE_SECURE=false`, and `PURESOC_AUTH_REQUIRE_EMAIL_VERIFICATION=false`. When placing Compose behind HTTPS and real email delivery, set:
...
The compose file generates/fixes the internal database URL, database name, Postgres password, MinIO user/password, bucket name, Redis URL, service URLs, internal API trusted origin, and Microsoft callback URL derivation. The deployment FQDN is discovered from the incoming request `Host`, `Forwarded`, or `X-Forwarded-*` headers after the platform assigns it; it is not an env variable. Use `.env.example` only to override the local browser defaults or opt into live Microsoft 365 connector credentials:
...
`PURESOC_PERSISTENCE_MODE=memory` is the deterministic default used by tests and local contract runs. The Compose catalog sets the role-specific runtime values needed for the bundled stack, including `PURESOC_PERSISTENCE_MODE=prisma` and an internal `DATABASE_URL` for API, scheduler, and migrator containers. `.env.example` contains optional live Microsoft connector overrides that cannot be generated by the repository.
...
The Compose `puresoc-migrator` service runs `pnpm prisma:migrate:deploy` before the API starts. The `puresoc-api` service also runs the same idempotent deploy command before `pnpm start:api`, and the API process itself runs `npm run prisma:migrate:deploy` before listening in Prisma mode. These layers cover deployment runners that skip one-shot services, ignore `depends_on: service_completed_successfully`, or start the Dockerfile `CMD` directly before serving registration and other Prisma-backed routes. None of these paths resets the database or removes the named `puresoc-postgres-data` volume. Operators that run migrations through a separate release job can set `PURESOC_API_RUN_MIGRATIONS_ON_STARTUP=false`.
...
Dockerfiles under `infra/docker/` run workspace entrypoint scripts. API, web, and report-renderer start implemented HTTP processes. Worker, scheduler, and connector-runner start typed job-runtime loops. The Compose migrator reuses the API image to run `pnpm prisma:migrate:deploy`, the Compose API service defensively runs that same idempotent migration command before `pnpm start:api`, and API startup runs `npm run prisma:migrate:deploy` in Prisma mode before it binds the HTTP port.
