...
- ADR-016 defines the current locale strategy: `en` and `ro` are supported contract locales, legal-caveat copy falls back to English until Romanian wording is product/legal-approved, and future country-pack notification drafts should use generic `NotificationDraft.payloadJson` envelopes with versioned schema keys.
